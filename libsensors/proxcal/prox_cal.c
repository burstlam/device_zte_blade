...
const char *delim = "#;,\\n";
    ...
    while ((argc_c = getopt(argc, argv, "cdh")) != -1){
        ...
        case 'd':
            ...
            rv = ioctl(fd, PS_ALS_IOCTL_CONFIG_GET, &cfg);
        ...
        case 'c':
            ...
            rv = ioctl(fd, PS_ALS_IOCTL_PROX_ON);          // Turn on prox sensor
            if (rv)
                fail();
            sleep(1);                                      // Give it a second to turn on
            rv = ioctl(fd, PS_ALS_IOCTL_PROX_CALIBRATE);   //Calibrate sensor
            if (rv)
                fail();
            rv = ioctl(fd, PS_ALS_IOCTL_PROX_OFF);         // Turn off prox sensor
            if (rv)
                fail();

            rv = ioctl(fd, PS_ALS_IOCTL_CONFIG_GET, &cfg); // Get new settings
            if (rv)
                fail();
